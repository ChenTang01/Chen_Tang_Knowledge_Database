...
\ex{
    Give a two-dimensinal example for random variable, sample point, Statistic, observation of Statistic, sample space, image and $A_t$.
}{
...
The main contents of this chapter is the notes of \cite{angrist2014mastering}, \cite{angrist2009mostly}.
...
\proo{}{
Take the expectation of $X_i$ at both side:
\eq{
    \begin{align}
...
\tab{
    \item The communication time and I/O time are excluded in this fomula;
...
\tab{
    ...
    \item GPU has high throughput, it works well on application with \tit{massive parallelism}.
}
...
\re{"Deadly Trials"\lis{
        ...
        \item Delayed consequences may cause RL algorithm perform poorly.
    }}
...
\tit{optimal policy} denoted by $\pi_*$ (doesn't hold for partial observed MDP). They share the same state-value function, called the \tit{optimal state-value function}:
...
\cite{gans2007simple} investigates the posit of some bandit consumer choice model:
